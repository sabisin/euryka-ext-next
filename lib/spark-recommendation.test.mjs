...
describe("Spark catalog", () => {
  test("deduplicates sparks and merges their group names", () => {
    ...
    expect(catalog.find((item) => item.id === "rewrite")?.groups).toEqual([
      "Writing",
      "Popular",
    ]);
...
describe("Spark recommendation", () => {
  test("builds a prompt containing the authoritative spark IDs", () => {
    const prompt = buildSparkRecommendationUserPrompt("Summarize this", buildSparkCatalog([
      { title: "Popular", sparks },
    ]));
